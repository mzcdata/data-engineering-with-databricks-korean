...
-- MAGIC 
-- MAGIC 
-- MAGIC # Databricks의 뷰 및 CTE, 계속'(Views and CTEs on Databricks, Cont')
-- MAGIC 
-- MAGIC 노트북  [DE 3.2A - Views and CTEs on Databricks]($./DE 3.2A - Views and CTEs on Databricks)를 통해 다음 두 가지 표 목록을 검토했습니다 <br>
-- MAGIC 그리고 우리의 Global Temp view **`global_temp_view_dist_gt_1000`** 이 첫 번째 목록에 포함되지 않았다는 특별한 주의가 있는 뷰들. <br>
-- MAGIC 
...
-- MAGIC 
-- MAGIC 
-- MAGIC 그러나 기본적으로 데이터베이스를 사용하도록 이 세션을 구성해야 합니다. <br>
...
-- MAGIC 
-- MAGIC 
-- MAGIC 마지막으로 다음 두 셀을 실행하여 확인합니다: <br>
-- MAGIC 1. 테이블 **`external_table`** 이 여전히 존재한다.
-- MAGIC 2. view **`view_delays_abq_lax`** 가 여전히 존재한다.
-- MAGIC 3. temp view **`temp_view_delays_gt_120`** 가 **존재하지 않는다**.
-- MAGIC 3. global temp view **`global_temp_view_dist_gt_1000`** 가 special **`global_temp`** database에 존재한다.
-- MAGIC 
...
-- MAGIC 
-- MAGIC <img src="https://files.training.databricks.com/images/icon_hint_24.png">힌트: 이전 노트북으로 돌아가서 SHOW TABLES를 실행하는 경우 <br>
-- MAGIC 다시 현재 데이터베이스의 세 테이블과 보기가 모두 계속 표시됩니다. <br>
...
-- MAGIC 
-- MAGIC 
-- MAGIC 앞서 언급한 것처럼 temp views 가 스파크 세션에 연결되어 있으므로 액세스할 수 없습니다.. <br>
-- MAGIC * 클러스터 재시작 후
-- MAGIC * 클러스터 분리 및 재연결 후
-- MAGIC * Python 패키지를 설치한 후 Python 인터프리터를 다시 시작합니다
-- MAGIC * 또는 다른 노트북에서
-- MAGIC 
-- MAGIC ...글로벌 임시 뷰를 제외한 특별한 예외가 있습니다.
-- MAGIC 
-- MAGIC 
...
-- MAGIC 
-- MAGIC 
-- MAGIC Global temp views는 다른 temporary views와 매우 유사하게 동작하지만 한 가지 중요한 방식이 다릅니다. <br>
-- MAGIC Global temp views behave much like other temporary views but differ in one important way. 
-- MAGIC 
-- MAGIC 클러스터에 있는 **`global_temp`** 데이터베이스에 추가됩니다.<br>
-- MAGIC They are added to the **`global_temp`** database that exists on the **`cluster`**.
-- MAGIC 
-- MAGIC 클러스터가 실행되는 동안 데이터베이스는 유지되고 클러스터에 연결된 모든 노트북은 글로벌 임시 보기에 액세스할 수 있습니다.<br>
...
-- MAGIC   
-- MAGIC 다음 셀을 실행하여 이 기능을 확인할 수 있습니다:<br>
...
-- MAGIC 
-- MAGIC 
-- MAGIC 클러스터가 다시 시작되면 Global temp views가 "손실"됩니다. <br>
-- MAGIC Global temp views are "lost" when the cluster is restarted.
-- MAGIC 
-- MAGIC 지금 하지 마십시오. 하지만 클러스터를 다시 시작하면 테이블이 더 이상 존재하지 않기 때문에 위의 select 문이 실패합니다. <br>
...
-- MAGIC 
-- MAGIC  
-- MAGIC ## 공통 테이블 식 - Common Table Expressions (CTEs)
-- MAGIC 
-- MAGIC CTE는 다양한 맥락에서 사용될 수 있다. 아래는 질의에서 CTE를 사용할 수 있는 다양한 방법의 몇 가지 예입니다. 첫째, CTE를 사용하여 여러 개의 열 별칭을 만드는 예입니다. <br>
...
-- MAGIC %md
-- MAGIC 
-- MAGIC  다음은 CTE 정의에서 CTE의 예입니다. <br>
...
-- MAGIC %md
-- MAGIC 
-- MAGIC subquery 식에 CTE를 사용할 수도 있습니다.
...
-- MAGIC %md
-- MAGIC 
-- MAGIC  마지막으로 CREATE VIEW 문의 CTE입니다. <br>
