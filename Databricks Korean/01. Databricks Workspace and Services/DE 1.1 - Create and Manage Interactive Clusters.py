...
# MAGIC
# MAGIC Databricks 클러스터는 ETL 파이프라인, 스트리밍 분석, 애드혹 분석 및 머신러닝과 같은 데이터 엔지니어링, 데이터 사이언스 및 데이터 분석 워크로드를 실행하는 계산 리소스 및 구성 집합입니다. 이러한 워크로드는 노트북의 명령 집합 또는 자동 작업으로 실행됩니다.
# MAGIC
# MAGIC 데이터 블록은 All Purpose Cluster와 Job Cluster를 구분합니다.
# MAGIC - All Purpose Cluster를 사용하여 대화형 노트북을 사용하여 데이터를 공동으로 분석할 수 있습니다.
# MAGIC - Job Cluster를 사용하여 빠르고 강력한 자동 작업을 실행할 수 있습니다.
# MAGIC
...
# MAGIC ## 클러스터 생성
# MAGIC
# MAGIC 현재 작업 중인 Workspace에 따라 클러스터 생성 권한이 없을 수도 있습니다.
# MAGIC
# MAGIC 이 섹션의 지침에서는 클러스터 작성 **권한이 있으며 이 과정을 실행하려면 새 클러스터를 배포해야 한다고 가정합니다.**
# MAGIC
...
# MAGIC 1. **Create Cluster** 버튼을 클릭합니다.
...
# MAGIC
# MAGIC **Restart**, **Terminate**, 및 **Delete**의 효과는 다르지만 모두 클러스터 종료 이벤트입니다.
# MAGIC
