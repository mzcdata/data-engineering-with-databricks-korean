...
-- MAGIC # 데이터 랩 재구성
-- MAGIC 
-- MAGIC 이 실습에서는 각 사용자가 **`events`** 에서 특정 작업을 수행한 횟수를 집계하는 **`clickpaths`** 표를 만든 다음 이 정보를 이전 노트북에서 생성한 **`transactions`** 의 평평한 보기에 연결합니다.
-- MAGIC 
-- MAGIC 또한 항목 이름에서 추출한 정보를 기반으로 **`sales`** 에 기록된 항목에 플래그를 지정하는 새로운 상위 함수를 탐색할 수 있습니다.
-- MAGIC 
-- MAGIC ## 학습 목표
-- MAGIC 이 실습을 완료하면 다음을 수행할 수 있습니다:
-- MAGIC - 피벗 및 조인 테이블을 사용하여 각 사용자에 대한 클릭 경로 생성
-- MAGIC - 구매한 제품의 플래그 유형에 고차 함수 적용
-- MAGIC 
...
-- MAGIC %md
-- MAGIC ## 클릭 경로를 생성하도록 데이터 세트 모양 변경
-- MAGIC 이 작업은 사용자가 사이트에서 수행한 모든 작업과 최종 주문이 어떻게 표시되었는지에 대한 기록을 만들기 위해 **`events`** 및 **`transactions`** 테이블의 데이터를 결합합니다.
-- MAGIC 
-- MAGIC **`clickpaths`** 테이블에는 **`transactions`** 테이블의 모든 필드와 자체 열에 있는 모든 **`event_name`** 의 카운트가 포함되어야 합니다. 구매를 완료한 각 사용자는 최종 표에 한 줄씩 표시해야 합니다. 먼저 **`events`** 테이블을 회전하여 각 **`event_name`** 에 대한 카운트를 가져옵니다.
-- MAGIC 
...
-- MAGIC 
-- MAGIC 
-- MAGIC ### 1. 피벗 **`events`** 를 사용자별로 카운트합니다
-- MAGIC 우리는 각 사용자가 **`event_name`** 열에 지정된 특정 이벤트를 수행한 횟수를 집계하고자 한다. 이를 위해 **`user`** 로 그룹화하고 **`event_name`** 에서 피벗하여 모든 이벤트 유형의 카운트를 자체 열에 제공하면 아래 스키마가 생성됩니다.
-- MAGIC 
...
-- MAGIC %md
-- MAGIC 
-- MAGIC **참고**: Python을 사용하여 랩 전체에서 가끔 검사를 실행합니다. 아래 도우미 기능은 지침을 따르지 않은 경우 변경해야 할 사항에 대한 메시지와 함께 오류를 반환합니다. 출력이 없으면 이 단계를 완료한 것입니다.
-- MAGIC 
...
-- MAGIC %md
-- MAGIC 
-- MAGIC 아래 셀을 실행하여 보기가 올바르게 작성되었는지 확인하십시오.
-- MAGIC 
...
-- MAGIC %md
-- MAGIC 
-- MAGIC ### 2. 모든 사용자의 이벤트 수 및 트랜잭션 Join
-- MAGIC 
-- MAGIC 그런 다음 **`events_pivot`** 와 **`transactions`** 를 결합하여 **`clickpaths`** 테이블을 만듭니다. 이 테이블은 아래와 같이 위에 작성된 **`events_pivot`** 테이블의 이벤트 이름 열과 **`transactions`** 테이블의 열이 같아야 합니다.
-- MAGIC 
...
-- MAGIC %md
-- MAGIC 
-- MAGIC ## 구매한 제품의 플래그 유형
-- MAGIC 여기서 상위 함수 **`EXISTS`** 를 사용하여 구입한 품목이 매트리스인지 베개 제품인지를 나타내는 부울 열 **`mattress`** 및 **`pillow`** 를 생성합니다.
-- MAGIC 
-- MAGIC 예를 들어, **`items`** 열의 **`item_name`** 이 문자열 **`"Mattress"`** 로 끝나는 경우 **`mattress`** 의 열 값은 **`true`** 이고 **`pillow`** 의 값은 **`false`** 이어야 합니다. 다음은 항목과 결과 값의 몇 가지 예입니다.
-- MAGIC 
...
-- MAGIC 
-- MAGIC <a href="https://docs.databricks.com/sql/language-manual/functions/exists.html" target="_blank">exists</a> 기능에 대한 자세한 내용은 설명서를 참조하십시오. <br>
-- MAGIC 조건식 **`item_name LIKE "%Mattress"`**  사용하여 **`item_name`** 문자열이 "Mattress" 단어로 끝나는지 확인할 수 있습니다.
-- MAGIC 
