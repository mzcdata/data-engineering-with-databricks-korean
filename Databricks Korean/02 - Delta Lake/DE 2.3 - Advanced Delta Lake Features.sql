...
-- MAGIC
-- MAGIC 아래 셀은 이전 레슨의 모든 트랜잭션을 단일 셀에서 실행합니다. (**`DROP TABLE`** 제외!) <br/>
...
-- MAGIC 실제로 Delta Lake 테이블은 클라우드 객체 스토리지에 저장된 파일 모음에 의해 백업됩니다. <br/>
...
-- MAGIC
-- MAGIC **참고**: Delta Lake와 함께 작동하기 위해 이러한 파일에 대한 모든 것을 아는 것이 지금 당장 중요하지는 않지만, 기술이 어떻게 구현되는지에 대한 이해를 높이는 데 도움이 될 것입니다. <br/>
...
-- MAGIC %md
-- MAGIC
-- MAGIC 각 트랜잭션은 새 JSON 파일을 델타 레이크 트랜잭션 로그에 기록합니다. 여기서 이 테이블에 대해 총 8개의 트랜잭션이 있음을 확인할 수 있습니다(Delta Lake는 0 인덱스). <br/> (Delta Lake is 0 indexed).
